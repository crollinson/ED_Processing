...
# sitelon <- -72.18
# 
# start.run <- as.Date("1850-01-01", "%Y-%m-%d") # Just for the trial
# start.loop <- as.Date("1900-01-01", "%Y-%m-%d") # Just for the trial
# 
# end.run <- as.Date("3010-12-31", "%Y-%m-%d") # Just for the trial
# end.loop <- as.Date("1909-12-31", "%Y-%m-%d") # Just for the trial
# 
# # bins <- as.numeric(c(strftime(start.loop, '%Y'), seq(from=as.numeric(paste0(substr(as.numeric(strftime(start.loop, "%Y"))+100, 1, 2), "00")), to=as.numeric(strftime(end.loop, '%Y')), by=100))) # Creating a vector with 100 year bins for the time period of interest
# bins <- seq(as.numeric(strftime(start.loop, '%Y')), as.numeric(strftime(end.loop, '%Y')), by=5)
# 
...
## --------------------------------------------------------------
## --------------------------------------------------------------
model2netcdf.ED2.paleon <- function(site, raw.dir, new.dir, sitelat, sitelon, start.run, start.loop, end.run, end.loop, bins) {
...
Var.Desc <- c("Dead", "Leaf", "Root", "Sapwood Aboveground", "Sapwood Belowground", "Seeds", "Wood Storage", "CWD", "Soil C, Fast", "Soil C, Slow")
...
## ----------------
	...
	if(y < length(bins)){
		...
		end <- as.numeric(strftime(end.loop, "%Y")) 		
	}
...
#	print(paste0("-------------------  Year: ", start, " - ", end, "  ----------")) 
	  ...
	  ## ----------------
    # General info
    npatch <- getHdf5Data(ncT, "NPATCHES_GLOBAL")
    ncohort <- getHdf5Data(ncT, "NCOHORTS_GLOBAL")
    
	  # Cohort-Level Info
		nplant <- getHdf5Data(ncT, "NPLANT")
		pft <- getHdf5Data(ncT, "PFT")
    pftT <- sort(unique(pft)) # listing the PFTs present, in order

		# Patch-Level Info:
		patch.n <- getHdf5Data(ncT, 'PACO_N') 
		patch.start <- getHdf5Data(ncT, 'PACO_ID')
		patch.area <- getHdf5Data(ncT, 'AREA')
...
#    patch.area.co.sum <- sum(patch.area.co)
	  ...
	  ## ----------------
      	...
      	agb <- cbind(getHdf5Data(ncT, 'AGB_CO')*dens.co[,1], pft, patch.co) # AGB by cohort; units: kgC/plant x plant/m2 = kgC/m2
        ...
        agb4 <- tapply(agb3$agb, list(agb3$pft), FUN=weighted.mean, w=patch.area[patch.n>0]);  # relative PFT AGB within a patch; Units: fraction (unitless)
...
#         fcomp <- fcomp[ordered(as.numeric(paste(fcomp$PFT))),] # neccessary if reading PFT as categorical
        ...
        ## -- NOTE: right now the units are cm2/plant --> use nplant to get area    
		    ...
		    ba <- cbind(getHdf5Data(ncT, 'BA_CO')*dens.co[,1], pft, patch.co) # BA by cohort; note: cm2/m2 = m2/ha
		    ba2 <- aggregate(ba[,1], by=list(ba[,2], ba[,3]), sum); names(ba2) <- c("pft", "patch", "ba") # total PFT BA within a patch; units: m2/ha
		    ba3 <- tapply(ba2$ba, list(ba2$pft), FUN=weighted.mean, w=patch.area[patch.n>0]);  # PFT BA weighted by patch
        ...
        dens3 <- tapply(dens2$dens, list(dens2$pft), FUN=weighted.mean, w=patch.area[patch.n>0]);  # PFT BA weighted by patch; units: plants/m2
      ...
      for(p in unique(Var.ED[1:7])){
        Cpools.co[,p] <- getHdf5Data(ncT, p)*dens.co[,1] # units: kgC/plant x plants/m2 = kgC/m2
        Cpools[p] <- sum(tapply(Cpools.co[,p], list(Cpools.co[,8]), sum)*patch.area[patch.n>0])
      }

      Cpools["CWD"] <- getHdf5Data(ncT, 'MMEAN_CWD_C_PY') # units: kgC/m2
      Cpools["FAST_SOIL_C"] <- getHdf5Data(ncT, 'MMEAN_FAST_SOIL_C_PY') # units: kgC/m2
      Cpools["SLOW_SOIL_C"] <- getHdf5Data(ncT, 'MMEAN_SLOW_SOIL_C_PY') # units: kgC/m2

      out <- add(array(Cpools), 8)
      ...
      live <-  cbind(getHdf5Data(ncT, 'BALIVE')*dens.co[,1], pft, patch.co) # AGB by cohort; units: kgC/plant x plant/m2 = kgC/m2
      ...
      out <- add(live.tot, 9) # units: kgC/m2

	  	# TotSoilCarb - total soil & litter content over entire profile
		  out <- add(sum(Cpools[c("FAST_SOIL_C", "SLOW_SOIL_C")]), 10) # units: kgC/m2
      ...
      dbiomass.co[,"DAGB_DT"] <- getHdf5Data(ncT, "DAGB_DT")*dens.co[,1]
      dbiomass.co[,"DBA_DT"] <- getHdf5Data(ncT, "DBA_DT")*dens.co[,1]
      dbiomass <- aggregate(dbiomass.co[,c("DAGB_DT", "DBA_DT")], by=list(dbiomass.co$patch), sum); names(dbiomass) <- c("patch", "DAGB_DT", "DBA_DT")
      
      out <- add(sum(dbiomass["DAGB_DT"]*patch.area[patch.n>0]), 11)
      out <- add(sum(dbiomass["DBA_DT"]*patch.area[patch.n>0]), 12)
	  ...
	  ## ----------------
		...
		out <- add(getHdf5Data(ncT, "MMEAN_GPP_PY")*yr2sec, 13) # original untis: kgC/m2/yr

		# AutoResp - Autotrophic respirtation
		out <- add(getHdf5Data(ncT, "MMEAN_PLRESP_PY")*yr2sec, 14) # original units: kgC/m2/yr

		# HeteroResp - Heterotrophic respiration
		out <- add(getHdf5Data(ncT, "MMEAN_RH_PY")*yr2sec, 15) # original units: kgC/m2/yr

		# NPP - Net Primary Productivity
		out <- add(getHdf5Data(ncT, "MMEAN_NPP_PY")*yr2sec, 16) # original units: kgC/m2/yr

		# NEE - Net Ecosystem Exchange
		out <- add(getHdf5Data(ncT, "MMEAN_NEP_PY")*yr2sec, 17) # original units: kgC/m2/yr

		# Fire - Fire emissions - need to calculate from Ignition Rate
		out <- add(getHdf5Data(ncT, "IGNITION_RATE")*mo2sec, 18) # original units: kgC/m2/month (I think, definitely kgC/m2)
		
	  ## ----------------
	  ## Energy Fluxes
	  ## ----------------
	  	# LW_albedo - Longwave Albedo
  		albedo.lw <- getHdf5Data(ncT, "MMEAN_RLONG_ALBEDO_PY")
  		out <- add(albedo.lw, 19)

	  	# SW_albedo - Shortwave Albedo -- NOT FOUND
	  	out <- add(array(getHdf5Data(ncT, "MMEAN_ALBEDO_PY")), 20)
	  	
	  	# LWnet - Net Longwave Radiation
		  out <- add(array(getHdf5Data(ncT, "MMEAN_ATM_RLONG_PY")-getHdf5Data(ncT, "MMEAN_RLONGUP_PY")), 21) # Units: W/m2
	  	
	  	# SWnet - Net Shortwave Radiation (incoming - upgoing)
      out <- add(array(getHdf5Data(ncT, "MMEAN_ATM_RSHORT_PY")-getHdf5Data(ncT, "MMEAN_RSHORTUP_PY")), 22) # Units: W/m2


	  	# Qh - Sensible Heat -  ATM -> CAS
	  	out <- add(getHdf5Data(ncT, "MMEAN_SENSIBLE_AC_PY"), 23) # Units: W/m2
	  	
	  	# Qle - Latent Heat = Evapotranspiration
      out <- add(getHdf5Data(ncT, "MMEAN_VAPOR_AC_PY")*2.26e6, 24) # units: kg/m2/s x 2.5e6 (J/kg = lambda) / 1 (W = J/s)
      ...
      lai.co <- data.frame(cbind(getHdf5Data(ncT, "LAI_CO"), patch.co)); names(lai.co) <- c("LAI", "patch")
      lai <- aggregate(lai.co[,"LAI"], by=list(lai.co[,"patch"]), sum); names(lai) <- c("patch", "LAI")
      out <- add(sum(lai[,"LAI"]*patch.area[patch.n>0]), 25)

	  	# Qs - Surface Runoff
  		out <- add(getHdf5Data(ncT, "MMEAN_RUNOFF_PY"), 26)

	  	# Qsb - Subsurface Runoff (drainage + lateral flow); no lateralflow found
	  	out <- add(getHdf5Data(ncT, "MMEAN_DRAINAGE_PY"), 27)

	  	# Evap - Total Evaporation
		  out <- add(sum(getHdf5Data(ncT, "MMEAN_VAPOR_GC_PY"), getHdf5Data(ncT, "MMEAN_VAPOR_LC_PY"), getHdf5Data(ncT, "MMEAN_VAPOR_WC_PY"), getHdf5Data(ncT, "MMEAN_TRANSP_PY")), 28) # units: kg/m2/s

	  	# Transp - Total Transpriation
  		out <- add(getHdf5Data(ncT, "MMEAN_TRANSP_PY"), 29) # Units: kg/m2/s

	  	# SnowDepth - Total snow depth --> SFCW_DEPTH
      # Supplementing SFCW Depth (snow + water)
	  	out <- add(getHdf5Data(ncT, "MMEAN_SFCW_DEPTH_PY"), 30) # units: m

	  	# SWE - Snow Water Equivalent 
      #   --> Suplementing SFCW MASS 
		  out <- add(getHdf5Data(ncT, "MMEAN_SFCW_MASS_PY"), 31) # units: kg/m2

	  	# SoilMoist - Soil Moisture
		  out <- add(getHdf5Data(ncT, "MMEAN_SOIL_WATER_PY")*100, 32) # units: m3/m3 * 100 kg/m3

	  	# SoilTemp - Soil Temperature
		  out <- add(getHdf5Data(ncT, "MMEAN_SOIL_TEMP_PY"), 33) # units: K
	  ...
	  ## ----------------
		  ...
		  out <- add(getHdf5Data(ncT, "MMEAN_ATM_RLONG_PY"), 34) # units: W/m2
	  	
	  	# swdown - Incoming shortwave radiation
		  out <- add(getHdf5Data(ncT, "MMEAN_ATM_RSHORT_PY"), 35) # units: W/m2
	  	
	  	# precipf - Preciptiation rate (mean)
		  out <- add(getHdf5Data(ncT, "MMEAN_PCPG_PY"), 36) # units: kg/m2/s
	  	
	  	# psurf - surface pressure
		  out <- add(getHdf5Data(ncT, "MMEAN_ATM_PRSS_PY"), 37) # units: Pa
	  	
	  	# qair - specific humidity
		  out <- add(getHdf5Data(ncT, "MMEAN_ATM_SHV_PY"), 38) # units: kg/kg
	  	
	  	# tair - air temperature
		  out <- add(getHdf5Data(ncT, "MMEAN_ATM_TEMP_PY"), 39) # units: K
	  	
	  	# wind - wind speed
		  out <- add(getHdf5Data(ncT, "MMEAN_ATM_VELS_SI"), 40) # untis: m/s
	  	
	  	# CO2 - CO2 concentration
		  out <- add(getHdf5Data(ncT, "MMEAN_ATM_CO2_PY"), 41) # units: umol/mol

      
		## ----------------
		nc_close(ncT)
#		nc_close(ncT.histo)
    rm(patch.area.co, patch.co, agb, agb2, agb3, agb4, fcomp, ba, ba2, ba3, ba.pft, dens.co, dens2, dens3, dens.pft, Cpools, Cpools.co, dbiomass.co, dbiomass, ncohort, npatch)
    }  ## end file (month) loop 
...
## ----------------
    ...
    var[[7]] <- ncvar_def("AGB", units="kg m-2", dim=list(dim.t), longname="Total Aboveground Biomass")
    var[[8]] <- ncvar_def("CarbPools", units="kg m-2", dim=list(dim.cpools, dim.t), longname="Carbon in Each Model Carbon Pool")
    var[[9]] <- ncvar_def("TotLivBiom", units="kg m-2", dim=list(dim.t), longname="Total Living Biomass (leaf + root + sapwood)")
    var[[10]] <- ncvar_def("TotSoilCarb", units="kg m-2", dim=list(dim.t), longname="Total Soil Carbon (fast + slow)")
    var[[11]] <- ncvar_def("GWBI", units="kg m-2 yr-1", dim=list(dim.t), longname="Gross Woody Biomass Increment (analgous to tree-ring derive biomass)")
    var[[12]] <- ncvar_def("BAI", units="cm2 m-2 yr-1", dim=list(dim.t), longname="Basal Area Increment (analgous to tree-ring meaurements)")
    var[[13]] <- ncvar_def("GPP", units="kg m-2 s-1", dim=list(dim.t), longname="Gross Primary Productivity")
    var[[14]] <- ncvar_def("AutoResp", units="kg m-2 s-1", dim=list(dim.t), longname="Autotrophic Respiration")
    var[[15]] <- ncvar_def("HeteroResp", units="kg m-2 s-1", dim=list(dim.t), longname="Heterotrophic Respiration")
    var[[16]] <- ncvar_def("NPP", units="kg m-2 s-1", dim=list(dim.t), longname="Net Primary Productivity") # NOTE: Not broken down by PFT
    var[[17]] <- ncvar_def("NEE", units="kg m-2 s-1", dim=list(dim.t), longname="Net Ecosystem Exchange")
    var[[18]] <- ncvar_def("Fire", units="kg m-2 s-1", dim=list(dim.t), longname="Fire Emissions; note: I think original units were kgC/m2/month and have been converted to KgC/m2/s here")
    var[[19]] <- ncvar_def("LW_albedo", units="", dim=list(dim.t), longname="Longwave Albedo")
    var[[20]] <- ncvar_def("SW_albedo", units="", dim=list(dim.t), longname="Shortwave Albedo")
    var[[21]] <- ncvar_def("LWnet", units="W m-2", dim=list(dim.t), longname="Net Longwave Radiation")
    var[[22]] <- ncvar_def("SWnet", units="W m-2", dim=list(dim.t), longname="Net Shortwave Radiation")
    var[[23]] <- ncvar_def("Qh", units="W m-2", dim=list(dim.t), longname="Sensible Heat Flux (ATM -> Canopy)")
    var[[24]] <- ncvar_def("Qle", units="W m-2", dim=list(dim.t), longname="Latent Heat Flux; note: I'm going from the model documentation, but this seems off")
    var[[25]] <- ncvar_def("LAI", units="m2 m-2", dim=list(dim.t), longname="Leaf Area Index")
    var[[26]] <- ncvar_def("Qs", units="kg m-2 s-1", dim=list(dim.t), longname="Surface Runoff")
    var[[27]] <- ncvar_def("Qsb", units="kg m-2 s-1", dim=list(dim.t), longname="Subsurface Runoff (Drainage)")
    var[[28]] <- ncvar_def("Evap", units="kg m-2 s-1", dim=list(dim.t), longname="Total Evaporation")
    var[[29]] <- ncvar_def("Transp", units="kg m-2 s-1", dim=list(dim.t), longname="Total Transpiration") # NOTE: not broken down by PFT
    var[[30]] <- ncvar_def("SnowDepth", units="m", dim=list(dim.t), longname="Total Snow/Water Depth (includes ponded rain)") # NOTE: Units differ from the protocol sheet
    var[[31]] <- ncvar_def("SWE", units="kg m-2", dim=list(dim.t), longname="Snow Water Equivalent (includes ponded rain)")
    var[[32]] <- ncvar_def("SoilMoist", units="kg m-3", dim=list(dim.soil, dim.t), longname="Soil Moisture") # NOTE: Units differ from the protocol sheet
    var[[33]] <- ncvar_def("SoilTemp", units="K", dim=list(dim.soil, dim.t), longname="Soil Temperature")
    var[[34]] <- ncvar_def("lwdown", units="W m-2", dim=list(dim.t), longname="Incoming Longwave Radiation")
    var[[35]] <- ncvar_def("swdown", units="W m-2", dim=list(dim.t), longname="Incoming Shortwave Radiation")
    var[[36]] <- ncvar_def("precipf", units="kg m-2 s-1", dim=list(dim.t), longname="Mean Precipitation Rate")
    var[[37]] <- ncvar_def("psurf", units="Pa", dim=list(dim.t), longname="Surface Pressure")
    var[[38]] <- ncvar_def("qair", units="kg kg-1", dim=list(dim.t), longname="Specific Humidity")
    var[[39]] <- ncvar_def("tair", units="K", dim=list(dim.t), longname="Air Temperature")
