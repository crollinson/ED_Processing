# A function to extract monthly variables from ED and put it into a netcdf file


# ----------------------------------------------------------------------
model2netcdf.ED2.simple <- function(site, raw.dir, new.dir, out.name, sitelat, sitelon, start.run) {
# ----------------------------------------------------------------------


## ------------------------------------ 
# Some useful functions, libraries etc
## ------------------------------------ 
library(ncdf4)
library(zoo)
...
# -------------------------
    ...
    ncT <- nc_open(file.path(raw.dir,flist[1])) # Opening the first hdf5 file in the output

	out[[1]] <- PFTs[,2]
	out[[2]] <- C.pools[,3]
	out[[3]] <- ncvar_get(ncT, "SLZ")

    nc_close(ncT)

for(i in 1:length(flist)){
	    ncT <- nc_open(file.path(raw.dir,flist[i])) 
...
#         fcomp <- fcomp[ordered(as.numeric(paste(fcomp$PFT))),] # neccessary if reading PFT as categorical
      ...
      out <- add(live.tot, 10) # units: kgC/m2
		  ...
		  out <- add(sum(Cpools[c("FAST_SOIL_C", "SLOW_SOIL_C")]), 11) # units: kgC/m2
...
## ----------------
    ...
    ## vector of dates included in the run
	  dates <- as.Date(paste0(substr(flist, index+3, index+11), "1"), "%Y-%m-%d")
...
nc <- nc_create(file.path(new.dir, paste(out.name, "nc", sep=".")), var)
